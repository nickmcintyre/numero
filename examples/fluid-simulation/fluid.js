class Fluid {
  ...
  constructor(baseColor = 'dodgerblue') {
  ...
  setTensors() {
    ...
    // Microscopic density
    this.state.n0 = this.fragment(num.ones(shape));
    this.state.nN = this.fragment(num.ones(shape));
    this.state.nS = this.fragment(num.ones(shape));
    this.state.nE = this.fragment(num.ones(shape));
    this.state.nW = this.fragment(num.ones(shape));
    this.state.nNE = this.fragment(num.ones(shape));
    this.state.nSE = this.fragment(num.ones(shape));
    this.state.nNW = this.fragment(num.ones(shape));
    this.state.nSW = this.fragment(num.ones(shape));
    // Macroscopic properties
    this.state.rho = this.fragment(num.ones(shape));
    this.state.ux = this.fragment(num.zeros(shape));
    this.state.uy = this.fragment(num.ones(shape).mult(this.params.u0));
  }

  // Break up a 2D tensor into pieces
  fragment(tensor) {
    const { m, n } = this.params;
  ...
  outlet() {
    ...
    ['nS', 'nSE', 'nSW'].forEach((quantity) => {
      ...
      this.state[quantity] = this.fragment(result[quantity]);
    });
  }
